...
// ============================================================================
constexpr int32_t MAX_DOCS_TO_LOAD = 6000;    // SciFact has ~5183 docs, dim=128 with ColBERTv2
    ...
    LoadFromJsonl(const std::string& jsonl_path, int32_t max_docs) {
        ...
        for (auto& tr : thread_results) {
            for (auto& doc : tr) {
                ...
                std::vector<float>().swap(doc.vecs);  // free immediately
            }
...
struct QueryDataWithGT {
    ...
    LoadFromJsonl(const std::string& jsonl_path, int32_t max_queries) {
        ...
        // Parse (queries are small, use fewer threads)
...
TEST_CASE("SciFact ColBERT: Direct vs MUVERA", "[scifact_emb_list]") {
    ...
    if (SKIP_DIRECT_TEST) {
        ...
        printf("\n");
        printf("[Direct] nDCG@10=%.4f, MRR@10=%.4f\n", calc_ndcg_vs_gt(direct_ids, 10), calc_mrr_vs_gt(direct_ids, 10));
        fflush(stdout);
    }
