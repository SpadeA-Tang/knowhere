#!/usr/bin/env python3
"""
Prepare MS MARCO BGE-M3 multi-vector embeddings for knowhere testing.

Usage:
    # Encode real MS MARCO data with BGE-M3 (requires GPU)
...
def encode_with_bgem3(texts: list, model, batch_size: int = 12, max_length: int = 512):
    """Encode texts using BGE-M3 ColBERT mode. Returns (vectors, offsets)."""
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Encoding"):
        ...
        output = model.encode(batch, batch_size=batch_size, max_length=max_length,
        ...
        for vecs in output['colbert_vecs']:
            ...
            offsets.append(offsets[-1] + len(valid))
...
def download_and_encode_msmarco(max_docs: int, max_queries: int, output_dir: Path):
    """Download MS MARCO and encode with BGE-M3."""
    ...
    # Encode passages
    print(f"\n=== Encoding {len(passages)} passages with BGE-M3 ===")
    doc_vectors, doc_offsets = encode_with_bgem3(passages, model)
    ...
    # Encode queries
    print(f"\n=== Encoding {len(queries)} queries with BGE-M3 ===")
    query_vectors, query_offsets = encode_with_bgem3(queries, model)

    return doc_vectors, doc_offsets, query_vectors, query_offsets


def generate_synthetic_data(max_docs, max_queries, dim=1024):
    """Generate synthetic BGE-M3-like data for testing."""
...
def main():
    parser = argparse.ArgumentParser(description="Prepare MS MARCO BGE-M3 data")
