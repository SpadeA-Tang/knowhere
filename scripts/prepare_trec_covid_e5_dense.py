#!/usr/bin/env python3
"""
Prepare TREC-COVID dense embeddings using E5-base-v2.

Reads from JSONL files produced by prepare_trec_covid_colbertv2.py to ensure
the same documents and queries are used for both dense and multi-vector evaluation.

Usage:
    python scripts/prepare_trec_covid_e5_dense.py --input-dir build --output-dir build
    python scripts/prepare_trec_covid_e5_dense.py --input-dir build --output-dir build --model /path/to/e5
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare TREC-COVID dense embeddings with E5-base-v2"
    )
    parser.add_argument("--input-dir", type=str, required=True,
                        help="Directory with trec_covid_gt_docs.jsonl and trec_covid_gt_queries.jsonl")
    ...
    docs_jsonl = input_dir / "trec_covid_gt_docs.jsonl"
    queries_jsonl = input_dir / "trec_covid_gt_queries.jsonl"

    if not docs_jsonl.exists() or not queries_jsonl.exists():
        print(f"ERROR: JSONL files not found in {input_dir}")
        print(f"Expected: trec_covid_gt_docs.jsonl, trec_covid_gt_queries.jsonl")
        print(f"Run prepare_trec_covid_colbertv2.py first.")
        return

    # Load texts from JSONL
    print("=== Loading texts from JSONL ===")
    doc_texts = []
    ...
    save_dense_binary(doc_embeddings, str(output_dir / "trec_covid_e5_docs.bin"))
    del doc_embeddings

    # Encode queries
    print(f"\n=== Encoding {len(query_texts)} queries ===")
    query_embeddings = encoder.encode(query_texts, args.batch_size, args.max_length, prefix="query: ")
    save_dense_binary(query_embeddings, str(output_dir / "trec_covid_e5_queries.bin"))

    # Save GT (binary pids)
    save_gt_binary(gt_pids, str(output_dir / "trec_covid_e5_gt.bin"))

    # Save graded relevance (for nDCG)
    if has_rels:
        save_gt_rels_binary(gt_pids, gt_rels, str(output_dir / "trec_covid_e5_gt_rels.bin"))
    ...
    print(f"\nOutput files:")
    print(f"  {output_dir / 'trec_covid_e5_docs.bin'}")
    print(f"  {output_dir / 'trec_covid_e5_queries.bin'}")
    print(f"  {output_dir / 'trec_covid_e5_gt.bin'}")
    if has_rels:
        print(f"  {output_dir / 'trec_covid_e5_gt_rels.bin'}  (graded, for nDCG)")
